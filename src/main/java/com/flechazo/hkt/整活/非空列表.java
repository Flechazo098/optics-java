...
public final class 非空列表<值类型> implements 盒子<非空列表.型, 值类型>, Iterable<值类型> {
    ...
    public Iterator<值类型> iterator() {
        return new Iterator<>() {
            ...
            @Override public boolean hasNext() { return 在头 || 尾迭代器.hasNext(); }
            @Override public 值类型 next() {
                if (在头) { 在头 = false; return 头; }
                return 尾迭代器.next();
            }
        };
    }
    ...
    @Override
    public String toString() { return 转列表盒().值().toString(); }
