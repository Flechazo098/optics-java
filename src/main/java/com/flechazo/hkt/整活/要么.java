...
public sealed interface 要么<左类型, 右类型> extends 盒子<要么.右型<左类型>, 右类型> permits 要么.左, 要么.右 {
    ...
    record 左<左类型, 右类型>(左类型 错误) implements 要么<左类型, 右类型> {
        @Override public boolean 是左() { return true; }
        @Override public 左类型 取左() { return 错误; }
        @Override public 右类型 取右() { throw new IllegalStateException("左"); }
        @Override public <新右> 要么<左类型, 新右> 映射(功能<? super 右类型, ? extends 新右> 功能) {
            return new 左<>(错误);
        }
    }
